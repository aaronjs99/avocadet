...
from .analyzer import ColorAnalyzer, SizeEstimator, AvocadoAnalysis, Ripeness, SizeCategory
...
class LivestreamProcessor:
    ...
    def _process_frame(self, frame: np.ndarray) -> FrameResult:
        ...
        for detection in detections:
            ...
            # Estimate size
            size_category, relative_size = self.size_estimator.estimate(
                detection.bbox
            )
            
            analyses.append(AvocadoAnalysis(
        ...
        if show_window:
            ...
            if self.detector.segmenter is not None:
                cv2.createTrackbar("Min Area", window_name, 
                                  self.detector.segmenter.min_area, 5000, 
                                  lambda v: None)
                
                # Max area for segmentation (5000-100000)
                cv2.createTrackbar("Max Area", window_name, 
                                  min(self.detector.segmenter.max_area, 100000), 100000, 
                                  lambda v: None)
                
                # Circularity threshold (0-100, displayed as 0.0-1.0)
                cv2.createTrackbar("Circular %", window_name, 
                                  int(self.detector.segmenter.circularity_threshold * 100), 100, 
                                  lambda v: None)
        
        print(f"Starting avocado detection on source: {self.source}")
        print("Press 'q' to quit, 'p' to pause, 's' to save screenshot, 'f' to fullscreen")
        ...
        try:
            while self.running:
                ...
                if show_window:
                    ...
                    if self.detector.segmenter is not None:
                        self.detector.segmenter.min_area = cv2.getTrackbarPos("Min Area", window_name)
                        self.detector.segmenter.max_area = cv2.getTrackbarPos("Max Area", window_name)
